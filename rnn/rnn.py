"""
This code was adapted from the baseline RNN implementations found at https://github.com/chrisvdweth/nus-cs4248x/blob/master/3-neural-nlp/src/rnn.py
"""
import torch
...
class RnnLanguageModel(nn.Module):
    ...
    def __init__(self, params):
        ...
        if self.params.rnn_cell.upper() == "RNN":
            ...
            raise Exception(
                "[Error] Unknown RNN Cell. Currently supported: RNN, GRU, LSTM"
            )
        self.rnn = rnn(
    ...
    def generate(
        ...
        inputs = np.array(
            vocabulary.lookup_indices([start_token])
            + vocabulary.lookup_indices(seed_tokens)
        )
    ...
    def init_hidden(self, batch_size):
        if self.params.rnn_cell.upper() == "LSTM":
            return (
                torch.zeros(
                    self.params.rnn_num_layers, batch_size, self.params.rnn_hidden_size
...
class RnnTextClassifier(nn.Module):
    ...
    def __init__(self, params):
        ...
        linear_sizes = [
            params.rnn_hidden_size * self.rnn_num_directions * self.scaling_factor
    ...
    def forward(self, inputs, hidden):
        ...
        if self.params.rnn_cell == "LSTM":
            last_hidden = hidden[0].view(
                self.params.rnn_num_layers,
    ...
    def init_hidden(self, batch_size):
        if self.params.rnn_cell == "LSTM":
            return (
                torch.zeros(
                    self.params.rnn_num_layers * self.rnn_num_directions,
                    batch_size,
                    self.params.rnn_hidden_size,
                ),
                torch.zeros(
...
class Encoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.embedding = nn.Embedding(
            self.params.vocab_size_encoder, self.params.embed_size
        ).to(self.params.device)
        ...
        self.rnn = rnn(
            ...
            dropout=0 if self.params.rnn_num_layers == 1 else self.params.rnn_dropout,
            batch_first=True,
        ).to(self.params.device)
    ...
    def forward(self, X):
        ...
        # output: (batch_size, seq_len, hidden size)
    ...
    def _init_hidden(self, batch_size):
        if self.params.rnn_cell.upper() == "LSTM":
            return (
                torch.zeros(
                    self.params.rnn_num_layers * self.num_directions,
                    batch_size,
                    self.params.rnn_hidden_size,
                ).to(self.params.device),
                torch.zeros(
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Embedding):
                torch.nn.init.uniform_(m.weight, -0.001, 0.001).to(self.params.device)
            elif isinstance(m, nn.Linear):
                torch.nn.init.xavier_uniform_(m.weight).to(self.params.device)
                m.bias.data.fill_(0.01)
...
class Decoder(nn.Module):
    ...
    def __init__(self, params, criterion):
        ...
        # Embedding layer
        self.embedding = nn.Embedding(
            self.params.vocab_size_decoder, self.params.embed_size
        ).to(self.params.device)

        # Calculate number of directions of the encoder (not for the decoder!)
        self.encoder_num_directions = (
            2 if self.params.rnn_encoder_bidirectional == True else 1
        )
        ...
        self.out = nn.Linear(linear_sizes[-1], params.vocab_size_decoder).to(
            self.params.device
        )
    ...
    def forward(self, inputs, hidden, encoder_hidden_states, epoch=0):
        batch_size, num_steps = inputs.shape

        # Create SOS token tensor as first input for decoder
        token = torch.LongTensor([[self.params.special_token_sos]] * batch_size).to(
            self.params.device
        )

        # Decide whether to do teacher forcing or not
        teacher_forcing_prob = self.params.teacher_forcing_prob
        if self.params.teacher_forcing_schedule is not None:
            teacher_forcing_prob = self.params.teacher_forcing_schedule(epoch)
        use_teacher_forcing = random(1)[0] < teacher_forcing_prob
        ...
        for i in range(num_steps):
            output, hidden, attention_weights = self._step(
                token, hidden, encoder_hidden_states
            )

            # print(i,  inputs[:, i] != self.params.special_token_pad)
            # x = inputs[:, i]
            # mask = x != self.params.special_token_pad
            # if mask.sum() > 0:
            #     loss += self.criterion(output[mask], x[mask])
    ...
    def generate(self, hidden, encoder_hidden_states, max_len=100):
        ...
        token = torch.LongTensor(
            [[self.params.special_token_sos]] * encoder_hidden_states.shape[0]
        ).to(self.params.device)
        decoder_attentions = torch.zeros(
            max_len, encoder_hidden_states.shape[0], encoder_hidden_states.shape[1]
        )
        for i in range(max_len):
            ...
            if attention_weights is not None:
                decoder_attentions[i] = attention_weights.squeeze(1)
            topv, topi = output.data.topk(1)
            decoded_sequence.append(topi)
            token = topi.detach()
        return (
            torch.stack(decoded_sequence).squeeze(2).T.cpu().detach().tolist(),
            decoder_attentions.permute(1, 0, 2),
        )
        ...
        # for item in range(encoder_hidden_states.shape[0]):
        #     if self.params.rnn_cell.upper() == "LSTM":
        #         _hidden = h[:,item,:].unsqueeze(1).contiguous(), c[:,item,:].unsqueeze(1).contiguous()
        #     else:
        #         _hidden = h[:,item,:].unsqueeze(1).contiguous()

        #     decoded_sequence = []
        #     # Create SOS token tensor as first input for decoder
        #     token = torch.LongTensor([[self.params.special_token_sos]] * 1).to(self.params.device)
        #     # token = torch.LongTensor([[self.params.special_token_sos]] * encoder_hidden_states.shape[0]).to(self.params.device)

        #     decoder_attentions = torch.zeros(max_len, encoder_hidden_states.shape[1])

        #     initial_state = encoder_hidden_states[item:item+1]
        ...
        #         output, _hidden, attention_weights = self._step(token, _hidden, initial_state)
        ...
        #         topv, topi = output.data.topk(1)
        #         if topi.item() == self.params.special_token_eos or topi.item() == self.params.special_token_pad:
        #             break
        #         else:
        #             decoded_sequence.append(topi.item())
        #             token = topi.detach()
        #     sequences.append(decoded_sequence)
        #     attentions.append(decoder_attentions[:i])
...
class DotAttention(nn.Module):
    ...
    def forward(self, encoder_hidden_states, decoder_hidden_state):
        ...
        attention_weights = torch.bmm(
            encoder_hidden_states, decoder_hidden_state.unsqueeze(2)
        )
        ...
        context = torch.bmm(
            encoder_hidden_states.transpose(1, 2), attention_weights.unsqueeze(2)
        ).squeeze(2)
...
class BahdanauAttention(nn.Module):
    def __init__(self, hidden_size, encoder_hidden_size):
        ...
        self.W2 = nn.Linear(encoder_hidden_size, hidden_size)
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        # Add dimension to hidden
        hidden = hidden.unsqueeze(1)

        # Calculate attention scores
        # (batch_size, seq_len, hidden_size) + (batch_size, 1, hidden_size) ==> (batch_size, seq_len, hidden_size)
        ...
        # Calculate context vector
        # (batch_size, seq_len, hidden_size) @ (batch_size, seq_len, 1) ==> (batch_size, 1, hidden_size)
...
class RnnSequenceLabeller(nn.Module):
    ...
    def forward(self, X, hidden):
        ...
        outputs = outputs.reshape(
            batch_size, seq_len, self.num_directions, self.params.rnn_hidden_size
        )
